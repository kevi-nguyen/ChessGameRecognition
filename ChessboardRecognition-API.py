...
def get_board_state(image, src_points, dst_points):
    image = ChessboardRecognition().transform_image(image, src_points, dst_points)
    return ChessboardStateLogic().board_to_string(ChessboardRecognition().get_board_state(image))


@app.get("/get_src_points")
def find_chessboard_corners_green(frame):
